...
> 즉시 실행 함수는 함수 이름이 없는 `익명 함수를 사용하는 것이 일반적`이다. 함수 이름이 있는 기명 즉시 실행 함수도 사용할 수 있으나, 기명 함수는 함수 선언문이 아니라 함수 리터럴로 평가되며 함수 이름은 함수 몸체에서만 참조할 수 있는 식별자이므로 즉시 실행 함수를 다시 호출할 수는 없다.
